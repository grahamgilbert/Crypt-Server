...
if [ -z "$(docker-machine ls | grep crypt)" ]; then
  docker-machine create -d vmwarefusion --vmwarefusion-disk-size=500000 --vmwarefusion-boot2docker-url "https://github.com/boot2docker/boot2docker/releases/download/v1.8.2/boot2docker.iso" --vmwarefusion-memory-size=2048 --vmwarefusion-cpu-count=4 crypt
...
KEYSET=$CWD/keyset
docker build -t "macadmins/imagr-server" $CWD
docker run -d \
  --name="crypt" \
