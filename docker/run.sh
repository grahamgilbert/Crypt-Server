...
python3.6 generate_keyczart.py
...
if [ ! -z "$ADMIN_PASS" ] ; then
  python3.6 manage.py update_admin_user --username=admin --password=$ADMIN_PASS
else
  python3.6 manage.py update_admin_user --username=admin --password=password
fi
